...
public abstract class SystemSetting extends ContentObserver implements Listenable {
    ...
    protected SystemSetting(Context context, Handler handler, String settingName) {
        this(context, handler, settingName, ActivityManager.getCurrentUser());
    }

    protected SystemSetting(Context context, Handler handler, String settingName, int userId) {
        this(context, handler, settingName, userId, 0);
    }

    public SystemSetting(Context context, Handler handler, String settingName, int userId,
            int defaultValue) {
        super(handler);
        mContext = context;
    ...
    public int getValue() {
        return System.getIntForUser(mContext.getContentResolver(), mSettingName, mDefaultValue,
                mUserId);
    }
